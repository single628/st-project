      ...
      <div class="container">
        <router-link v-for="item in ordersList" :key="item.id" tag="section" :to="'/order_detail?id=' + item.id">
          ...
          <div class="price-container">
            <span>共{{ item.foods.length }}件商品，实付</span>
            <span>￥{{ item.total_price }}</span>
          </div>
          <div class="footer">
            <router-link :to="{ path: '/store', query: { id: item.restaurant.id } }" tag="span">再来一单</router-link>
            <router-link 
            v-if="!item.has_comment" class="make_comment" :to="{ path: '/order/comment', query: { order_id: item.id } }">评价
            </router-link>
          </div>
        </router-link>
      </div>
    ...
    if (this.username) {
      orders().then((response) => {
        if (response.data.status === 200) {
          ...
          if (!this.noOrder) {
            this.$nextTick(() => {
              new BScroll(this.$refs.orderWrapper, {
                click: true,
                probeType: 3,
              });
            });
