    ...
    <article v-for="(item, restaurant_id) in cartList" :key="restaurant_id">
      <section class="title">
        ...
        <span
          ...
          v-if="
            editStatus && deleteSelectFood[restaurant_id]['allSelect'] === true
          "
      ...
      >
        <!-- foodkey食物的下标 -->
        <div v-if="Number(foodKey)">
          <div class="foods">
            <!-- 编辑状态下的取消选中 -->
            <span
              class="selected delete-selected"
              v-if="
                editStatus && deleteSelectFood[restaurant_id][foodKey] === true
              "
      ...
      <div class="bottom" v-show="!editStatus">
        <span
          ...
          :class="{ active: !selectFood[restaurant_id]['totalPrice'] }"
          >去结算
        </span>
        <span class="total-price"
          >￥{{ selectFood[restaurant_id]["totalPrice"].toFixed(2) }}</span
        >
      </div>
    </article>
    <!-- 编辑状态下的删除功能 -->
...
export default {
  ...
  methods: {
    ...
    // 取消要删除的商品
    cancelSelectDelete(restaurant_id, foodKey) {
      //取消删除选中
    ...
    // 选中要删除的商品
    selectDelete(restaurant_id, foodKey) {
      //选中删除商品
    ...
    isAllSelect(newObj) {
      //判断商品是否全选中了 如果全选中那么商家头像左边的按钮对应选中
  ...
  created() {
    this.emptyCart = !Object.keys(this.cartList).length; //为真就是显示购物车空空如也
    Object.keys(this.cartList).forEach((restaurant_id) => {
      ...
      Object.keys(restaurant).forEach((data) => {
        if (Number(data)) {
          ...
          this.selectFood[restaurant_id]["totalPrice"] +=
            Number(restaurant[data]["price"]) * Number(restaurant[data]["num"]);
        }
      });
    });
  },
